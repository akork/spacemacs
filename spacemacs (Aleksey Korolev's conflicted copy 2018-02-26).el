...
(defun aking/view-pdf ()
  (interactive)
  (shell-command "open -a skim report.pdf"))
...
(defun aking/tex-template ()
  (interactive)
  (find-file (concat "/Users/Aleksey/Dropbox/Settings/latex-template/report.tex")))

(defun aking/tex-new (path)
  (interactive "sEnter path: ")
  (let ((pathh "./testtex"))
    (save-window-excursion (shell-command (concat  "/Users/Aleksey/Dropbox/Settings/scripts/tex-new.sh " "./" path))
                           )
    (set-main-project (concat (current-file-dir) path))
    (find-file (concat path "/report.tex"))
    ))
...
(setq LaTeX-verbatim-environments-local '("Verbatim" "cppL" "textL"))
